...
std::vector<std::string> g_fontsPath;
map<std::string, int> g_loadResourse;
...
// 			std::string sDevPath = "/mnt/data/";
// 			string sImgPath = sDevPath + "img/", sFontPath = sDevPath + "slaveFont";
// 			cbl::removeDir(sImgPath, false);
// 			cbl::removeDir(sFontPath, false);
...
// 			{
// 				if (0 == it->second) {
// 					sUrl = "http://" + CSlaveModule::get_instance().getHost() + "/slaveImg/";
// 					sDevPath = sImgPath;
// 				}
// 				else {
// 					sUrl = "http://" + CSlaveModule::get_instance().getHost() + "/slaveFont/";
// 					sDevPath = sFontPath;
// 				}
// 				nRet = httpC.get(sUrl + it->first, sDevPath + it->first);
// 				if (0 != nRet) {
...
// 					g_fontsPath.push_back(sDevPath + it->first);
...
// 				fontDir = getPrjDir() + "qpf2/";
// #endif

// 				std::vector<std::string> filePathList, fontList;
// 				if (UTILITY_NAMESPACE::CDir::listRelativeDir(sImageDir, "*.*", filePathList) < 0) {
// 					printf("%s:listRelativeDir image error\r\n", __FUNCTION__);
// 				}
// 				if (UTILITY_NAMESPACE::CDir::listRelativeDir(fontDir, "*.*", fontList) < 0) {
// 					printf("%s:listRelativeDir font error\r\n", __FUNCTION__);
// 				}
// 				hmiproto::hmiact dataproto;
...
// 						continue;
// 					}
// 					sendRedisdata->add_fonts(fontList[i]);
// 				}
...
int redisSlaveRecvFunc(const string& sChannel, const string& sMessage, void* lpParameter) {
	...
	// 	{
	// 		g_loadResourse.insert(pair<std::string, int>(redisData.imgs()[i], 0));
	// 	}
	// 	for (int i = 0; i < redisData.fonts_size(); i++)
	// 	{
	// 		g_loadResourse.insert(pair<std::string, int>(redisData.fonts()[i], 1));
	// 	}
	// 	g_bIsStartLoad = true;
...
void* runConnectToMasterHmi(void* arg) {
	...
	// 				//init screen to load font
	// 				// for (size_t i = 0; i < g_fontsPath.size(); i++)
	// 				// {
	// 				// 	HmiGui::loadFontToQt(g_fontsPath[i]);
	// 				// }
