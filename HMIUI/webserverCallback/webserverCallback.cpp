...
int startWebServer() {
	...
	std::string sImgPath, sFontPath, sFontDir, sSlaveImgPath, sSlaveFontPath;
#ifdef WIN32
	// 模拟非debug模式关闭
#ifndef _DEBUG
	return 0;
	...
	sSlaveFontPath = sSiteRoot + "/slaveFont";
	...
	CPlatShell::runCmd(str, NULL);
	str = cbl::format("ln -s /mnt/wecon/wecon/run/project/qpf2/* %s", sSlaveFontPath.c_str());
	CPlatShell::runCmd(str, NULL);
#endif
